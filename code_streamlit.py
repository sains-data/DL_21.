...
BEST_THR_DEFAULT = 0.75  # Threshold untuk inverted model: prob_bully >= 0.75 = BULLY (strict untuk accuracy)
try:
...
def predict_bully_sentence(text, model=None, tokenizer=tokenizer, maxlen=300, threshold=None):
    ...
    # Model outputs probability of NOT BULLYING class (inverted logic)
    # So we need to invert: prob_bully = 1.0 - prob
    # Then: prob_bully >= threshold = BULLY, else = NOT BULLY
    prob_bully = 1.0 - prob
    label = "BULLY" if prob_bully >= thr else "NOT BULLY"

    # DEBUG LOG
    print(f"[PREDICT] text='{text}' -> prob_not_bully={prob:.4f}, prob_bully={prob_bully:.4f}, thr={thr:.4f}, label={label}")
        ...
        if predict_button and user_input:
            ...
            with st.expander("🔍 Debug Information"):
                ...
                with col_d2:
                    ...
                    st.caption(f"Probability (NOT BULLY): {probability:.4f}")
                    st.caption(f"Probability (BULLY): {(1.0-probability):.4f}")
            ...
            with col2:
                if not is_bully:
                    ...
                    # Confidence Bar - untuk NOT BULLY (higher prob = more confident it's not bully)
                    st.markdown("### Keyakinan Model")
                    st.progress(probability)
                    st.markdown(f"<div style='text-align: center; color: #3FB950; font-size: 20px; font-weight: 600;'>{probability*100:.1f}% NOT BULLYING</div>", unsafe_allow_html=True)
                    ...
                    # Confidence Bar - untuk BULLY (higher 1-prob = more confident it's bully)
                    st.markdown("### Keyakinan Model")
